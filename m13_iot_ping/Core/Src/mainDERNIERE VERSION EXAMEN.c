...
/* -------------------------------------------------------------------------- */
/* Adresse I2C du module RTC (type / BQ32000) */
...
//Formate un message et le met en file d'attente (Thread-safe) au lieu d'afficher direct.
void UART_Log(const char *fmt, ...)
{
    UartMsg *m;
...
void send_presence_broadcast(void)
{
   ...
   /* pcb->so_options |= SOF_BROADCAST; */
    ...
    /* 3. Adresse de broadcast (adaptée à  réseau actuel) */
...
err_t tcp_server_recv(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err)
{
    ...
    // C'est la ligne la plus importante : elle regarde  variable globale
    ...
    if (my_alert_status == 1) {
        UART_Log("⚠️ ENVOIE LE STATUS 'ALERT' !\r\n");
    } else {
        UART_Log("✅ Envoie le status 'normal'.\r\n");
    }
    UART_Log("--------------------------------------------------\r\n");
    // =================================================================
...
// 💡 Callback appelée quand la connexion client est réussie
/**
 * Si le voisin reponds elle utilise la fonction response
 * json envoie
 * ne ferme pas la connection pour attendre la reponse
 */
err_t tcp_client_connected(void *arg, struct tcp_pcb *tpcb, err_t err)
{
    /* ----------------------------- Vérification ---------------------------- */
 ...
 * Traite le JSON envoyé par le serveur.
  ...
  *
  *   donc
 Elle récupère le message JSON envoyé par le voisin et nettoie le buffer.
Elle extrait les données clés : statut d'alerte, intensité RMS et l'heure.

Elle identifie le voisin grâce à la fin de son adresse IP (ex: .177).

Elle lui attribue un slot mémoire et sauvegarde l'événement dans la FRAM.

Elle déclenche l'alerte générale si toi et le voisin vibrez en même temps.

Elle ferme la connexion TCP pour terminer l'échange proprement.
  *
 * */
err_t tcp_client_recv_response(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err)
{
    ...
    // Calcul du Max et stockage RAM // on ne fait plus car chaque noeud connait deja son max
...
//ne sert plus à rien debut pour tester
void RTC_GetTime(uint8_t *hour, uint8_t *min, uint8_t *sec)
{
    uint8_t data[3];
...
/*Elle fabrique une chaîne de caractères contenant la date
et l'heure actuelles, standardisée pour être comprise universellement (norme ISO 8601)*/
void build_iso8601_timestamp(char *buf, size_t len)
{
    RTC_DateTime dt;
    RTC_GetDateTime(&dt);
...
/**
  ...
  *   Protocol Control Block - adresse ip port moi et dest etat
  *   broadcast
  */
/* -------------------------------------------------------------------------- */
/* ENVOI D’UNE REQUÊTE NTP (Client UDP)                    */
...
void Sync_Time_NTP(void)
{
    ...
    /* 4. Adresse IP (Remise à l'IP Google qui marchait bien ) */
...
/*
 * Filtrage : Elle vérifie si le message UDP reçu contient bien le mot-clé "presence".

Mise à jour : Si le voisin est déjà connu, elle actualise son timestamp (last_seen)
pour dire "il est toujours vivant".

Ajout : Si c'est un nouveau voisin, elle l'enregistre dans la première case vide
de la liste dynamic_node_list.

Sécurité : Elle protège l'accès à la liste avec un Mutex pour éviter les bugs
si deux tâches y touchent en même temps.
 * */
void presence_recv_cb(void *arg, struct udp_pcb *pcb, struct pbuf *p,
                      const ip_addr_t *addr, u16_t port)
{
...
/*
 Initialisation Réseau : Elle démarre la pile LwIP, attend l'adresse IP (DHCP) et synchronise
 l'heure exacte (NTP).

Attente Utilisateur : Elle met le système en attente jusqu'à l'appui sur le Bouton Bleu.

Mode MARCHE (Resume) : Quand on appuie, elle réveille toutes les autres tâches (Sismique, Presence,
Client...) et lit la mémoire FRAM pour afficher l'état initial.

Mode PAUSE (Suspend) : Si on rappuie, elle endort toutes les tâches et affiche
tout l'historique (les 10 derniers séismes locaux et voisins) sur l'UART pour analyse.
 * */
void StartDefaultTask(void const * argument)
{
  /* init code for LWIP */
  ...
  for(;;) {
      if (HAL_GPIO_ReadPin(USER_Btn_GPIO_Port, USER_Btn_Pin) == GPIO_PIN_SET) {
        ...
        if (!system_is_running) {
          ...
          int WIPE_FRAM = 0; // <--- METTRE A 1, FLASHER, PUIS REMETTRE A 0  supprimer fram
          ...
          if (WIPE_FRAM) {
              ...
              UART_Log("⚠️ Changer maintenant 'int WIPE_FRAM = 0;' dans le code et reflasher !\r\n");
  ...
  /* Infinite loop */
	...
	extern struct netif gnetif;
	    ...
	    for (;;)
	    {
	    	...
	    	//pour pas bloquer udp trop lngtps
	        ...
	        {
	            // Conversion IP LwIP vers String pour fonction existante
	            char ip_str[16];
...
//C'est la tâche qui active le mode Serveur de ta carte pour qu'elle puisse être contactée par les autres
void StartServerTask(void const * argument)
{
