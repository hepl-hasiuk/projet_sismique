...
err_t tcp_client_recv_response(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err)
{
    ...
    nevt.source_ip   = ip_suffix;  // <--- On sauvegarde l'IP ici !
    ...
    UART_Log("💾 Sauvegarde FRAM Voisin .%d : %d/%d %dh%d (%.2fV)\r\n",
             ip_suffix, nd, nmo, nh, nmin, neighbor_max_rms);
    ...
    for(;;) {
        if (HAL_GPIO_ReadPin(USER_Btn_GPIO_Port, USER_Btn_Pin) == GPIO_PIN_SET) {
          ...
          if (!system_running) {
            ...
            UART_Log("--- 📂 LECTURE MEMOIRE FRAM 📂 --- \r\n");

                        // 1. VOISINS
                        NeighborEvent nevt;
                        for (int i = 0; i < 3; i++)
                        {
                            uint32_t addr = FRAM_NEIGHBOR_BASE_ADDR + (i * FRAM_NEIGHBOR_STRIDE);
                            FRAM_Read(addr, (uint8_t*)&nevt, sizeof(nevt));

                            // On affiche si l'heure est cohérente (mémoire non vierge)
                            if (nevt.hour < 24) {
                                sprintf(msg, "📡 VOISIN (IP .%d) | Date: 20%02d-%02d-%02d %02d:%02d:%02d | Force: %.2f V\r\n",
                                        nevt.source_ip,    // <--- Affiche l'IP
                                        nevt.year, nevt.month, nevt.day,
                                        nevt.hour, nevt.min, nevt.sec,
                                        nevt.intensity);
                                UART_Log("%s", msg);
                            } else {
                                UART_Log("📡 Emplacement %d vide.\r\n", i);
                            }
                        }

                        // 2. LOCAL (MOI)
                        if(last.intensity > 0.0f && last.hour < 24) {
                            sprintf(msg, "🏠 MOI (LOCAL)     | Date: 20%02d-%02d-%02d %02d:%02d:%02d | Force: %.2f V\r\n",
                             last.year, last.month, last.day,
                             last.hour, last.min, last.sec,
                             last.intensity);
                             UART_Log("%s", msg);
                        } else {
                            UART_Log("🏠 Pas d'evenement local enregistre.\r\n");
                        }
            ...
            osThreadResume(clientTaskHandle);
                      ...
                      // -----------------------------------------------------------
                      //  MODE PAUSE (SYSTEM STOPPED)
                      // -----------------------------------------------------------
                      system_running = 0;
                      UART_Log("\r\n>>> SYSTEM STOPPED <<<\r\n");

                      // ===========================================================
                      // 1. AJOUTER CECI : LECTURE DE LA PREUVE EN FRAM
                      // ===========================================================
                      UART_Log("\r\n--- 📂 LECTURE CONTENU FRAM (Preuve Stockage) 📂 ---\r\n");

                      NeighborEvent nevt;
                      for (int i = 0; i < 3; i++)
                      {
                          // Calcul de l'adresse de chaque slot (0, 1, 2)
                          uint32_t addr = FRAM_NEIGHBOR_BASE_ADDR + (i * FRAM_NEIGHBOR_STRIDE);

                          // Lecture physique de la puce FRAM
                          FRAM_Read(addr, (uint8_t*)&nevt, sizeof(nevt));

                          // Si l'heure est < 24, c'est que des données valides sont écrites
                          if (nevt.hour < 24) {
                              char frame_msg[100];
                              sprintf(frame_msg, "   [Slot %d] Voisin IP .%d | %d/%d %02dh%02d | Force: %.2f V\r\n",
                                      i, nevt.source_ip, nevt.day, nevt.month, nevt.hour, nevt.min, nevt.intensity);
                              UART_Log("%s", frame_msg);
                          } else {
                              UART_Log("   [Slot %d] -- Vide --\r\n", i);
                          }
                      }
                      UART_Log("----------------------------------------------------\r\n");
                      // ===========================================================
                      ...
                      // 2. LA SUITE RESTE INCHANGEE (Historique RAM)
